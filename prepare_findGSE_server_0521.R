    ...
    if(ploidy <= 2){
      ...
      if(het_pos < 80){
        ...
        for (ploidy_ind in seq(ploidy)){
	      ...
	      avg_cov <- round(meanfit_old, digits = 2)
	      #avg_cov <- round(meanfit_old)#, digits = 1)
	      #avg_cov <- round((meanfit_old+avg_cov)/2, digits = 2)
              cat('avg_cov: ',avg_cov,'\n')
            ...
            #
	    cat('raw genome size is: ',round(genome_size, 2),' \n')
            genome_size_meanfit     = sum(histo_fit$V1 * (histo_fit$V2 / 1000000)) / (ploidy*meanfit_old) # tetraploid size: 3374.929 Mb, haploid size: 843.7321
	    cat('meanfit genomesize is: ',round(genome_size_meanfit, 2),' \n')
	    cat('average genomesize is: ', round((genome_size+genome_size_meanfit)/2,2), ' \n')
            #
            if(ploidy == 1){
              dev.off()
              break
